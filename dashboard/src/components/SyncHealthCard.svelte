<script lang="ts">
	...
	let { health, status, onrefresh }: { health: SyncHealth | null; status: 'loading' | 'loaded' | 'empty' | 'error'; onrefresh: () => void } =
		$props();
</script>
...
<Card ar="حالة المزامنة مع المتجر" en="Sync health status">
	...
	{:else if status === 'error'}
		<EmptyState ar="تعذر تحميل حالة المزامنة" en="Could not load sync status" />
	{:else if !health}
	...
	{:else}
		...
		<div class="grid">
			<div>
				<div class="muted">آخر مزامنة / Last sync</div>
			...
			<div>
				...
				<div class="big num">{health.pending_count + health.retry_count}</div>
			</div>
			<div>
		...
		<div class="muted note">
			...
			<br /><span style="font-size: 0.78rem;">Cloud-complete does not prove the Retail outbox is empty.</span>
		</div>
